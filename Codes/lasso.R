...
sim_data<-function(K,t,sparsity=0.3)
{
  sigma_error<-0.1^2 
...
l=1
j=1
mean_mae_sparse<-matrix(NA,nrow=3,ncol=3)
mean_mae_nonsparse<-matrix(NA,nrow=3,ncol=3)
mean_hit_rate<-matrix(NA,nrow=3,ncol=3)
for (K in c(5,15,30)) 
{ 
  j=1
...
mega_output_LASSO<- list(mean_mae_nonsparse,mean_mae_sparse,mean_hit_rate)

save(mega_output_LASSO,file="LASSO.RData")